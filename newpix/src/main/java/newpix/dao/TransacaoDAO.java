...
public class TransacaoDAO {
    ...
    public void criarTransacao(Usuario remetente, Usuario destinatario, double valor) throws SQLException {
        ...
        String sqlInsertTransacao = "INSERT INTO transacoes (remetente_id, destinatario_id, valor) VALUES (?, ?, ?)";
        ...
        try {
            ...
            throw new SQLException("Erro ao processar a transação.", e);
        } finally {
            if (conn != null) {
                try {
    ...
    public List<Map<String, Object>> getExtratoPorPeriodo(int usuarioId, String dataInicial, String dataFinal) throws SQLException {
        ...
        String sql = "SELECT t.valor, t.data, " +
                     "CASE WHEN t.remetente_id = ? THEN 'enviada' ELSE 'recebida' END as tipo, " +
                     "CASE WHEN t.remetente_id = ? THEN dest.nome ELSE rem.nome END as outro_participante " +
                     "FROM transacoes t " +
                     "JOIN usuarios rem ON t.remetente_id = rem.id " +
                     "JOIN usuarios dest ON t.destinatario_id = dest.id " +
                     "WHERE (t.remetente_id = ? OR t.destinatario_id = ?) AND t.data BETWEEN ? AND ? " +
        ...
        try (Connection conn = DatabaseConfig.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    transacao.put("data", rs.getTimestamp("data").getTime());
